...
export const accounts = pgTable(
  ...
  (table) => ({
    providerAccount: uniqueIndex("account_provider_account_idx").on(
      table.providerId,
      table.accountId,
    ),
  }),
...
export const contentItems = pgTable(
  "content_item",
  {
    ...
    smartObjects: boolean("smart_objects").default(false).notNull(),
    editableText: boolean("editable_text").default(false).notNull(),
  ...
  (table) => ({
    catalogIndex: index("content_catalog_idx").on(
      table.kind,
      table.moderationStatus,
      table.createdAt,
    ),
    categoryIndex: index("content_category_idx").on(
      table.categoryId,
      table.moderationStatus,
    ),
  }),
...
export const contentAssets = pgTable(
  ...
  (table) => ({
    contentIndex: index("content_asset_item_idx").on(
      table.contentItemId,
      table.role,
    ),
  }),
...
export const favorites = pgTable(
  ...
  (table) => ({
    pk: primaryKey({ columns: [table.userId, table.contentItemId] }),
  }),
);
export const downloads = pgTable(
  ...
  (table) => ({
    contentIndex: index("download_content_idx").on(
      table.contentItemId,
      table.createdAt,
    ),
  }),
...
export const collections = pgTable(
  ...
  (table) => ({ userIndex: index("collection_user_idx").on(table.userId) }),
);
export const collectionItems = pgTable(
  ...
  (table) => ({
    pk: primaryKey({ columns: [table.collectionId, table.contentItemId] }),
  }),
);
export const subscriptions = pgTable(
  ...
  (table) => ({
    userIndex: index("subscription_user_idx").on(table.userId, table.status),
  }),
);

export const premiumWaitlist = pgTable(
  "premium_waitlist",
  {
    id: text("id").primaryKey(),
    email: text("email").notNull().unique(),
